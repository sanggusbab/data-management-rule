...
# 데이터 관리 규정
    ...
    규정들은 최소한의 행동 행동양식에 제한을 두는 것을 원칙으로 하며, HandS는 책임감 있게 심도있는 고민을 통해 규정을 제정하도록 한다.
    데이터 관리 규정은 문화적으로 정착되기에 오래 걸리고, 규정의 변화에 대해 데이터를 변화시키는데 투자되는 비용이 크므로 규정 제정의 전문성과 책임감 있는 참여의지가 필요하다.
    데이터 처리 의존성 문제가 있으므로 세부적인 내용 전달의 목적같은 필요한 상황을 제외하곤 영문을 활용함을 추천한다.
